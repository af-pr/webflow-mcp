...
class TestPlaceholderResolver:
    ...
    def test_env_placeholder_resolution(self, resolver, monkeypatch):
        monkeypatch.setenv("EXAMPLE_USER", "envuser")
        monkeypatch.setenv("EXAMPLE_PASS", "envpass")
        workflow = Workflow(
            name="test_env",
            steps=[
                Step(ActionType.FILL, {"value": "{{EXAMPLE_USER}}"}),
                Step(ActionType.FILL, {"value": "User: {{EXAMPLE_USER}}, Pass: {{EXAMPLE_PASS}}"}),
            ]
        )
    ...
    def test_param_priority_over_env(self, resolver, monkeypatch):
        ...
        workflow = Workflow(
            ...
            steps=[Step(ActionType.FILL, {"value": "{{EXAMPLE_USER}}"})]
        )
        
        resolved = resolver.resolve_workflow(workflow, {"EXAMPLE_USER": "paramuser"})
